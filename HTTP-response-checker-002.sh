...
# Create an associative array to store unique URLs based on their HTTP status codes
...
while read -r url; do
    ...
    if [ "$response" != "000" ]; then
        # Add the URL to the set to remove duplicates
        url_by_status["$response"]=$(echo -e "${url_by_status["$response"]}\n$final_url" | sort -u)
    fi
...
# Print unique URLs grouped by HTTP status code to the output file
